...
For ($i = 1; $i -le $n; $i++) {
    ...
    [xml]$doc = Get-Content (Join-Path $currentDir template.xml)

    for ($j = 1; $j -lt $i; $j++) {
        AddProjectReference $doc "..\ClassLibrary${j}\ClassLibrary${j}.csproj"
    }

    SaveProject $doc (Join-Path $currentDir "src\ClassLibrary${i}\ClassLibrary${i}.csproj")
}

[xml]$root = Get-Content (Join-Path $currentDir template.xml)
[xml]$rootLeft = Get-Content (Join-Path $currentDir template.xml)
[xml]$rootRight = Get-Content (Join-Path $currentDir template.xml)
...
SaveProject $root (Join-Path $currentDir "src\Root\Root.csproj")
SaveProject $rootLeft (Join-Path $currentDir "src\RootLeft\RootLeft.csproj")
SaveProject $rootRight (Join-Path $currentDir "src\RootRight\RootRight.csproj")
